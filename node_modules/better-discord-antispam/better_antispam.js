...
module.exports = async (client, options) => {
  ...
  const logChannel = (options && options.logChannel) || "spam-logy"; // Default value: "AhtiSpam-logs". Explication: This is the channel where every report about spamming goes to. If it's not set up, it will attempt to create the channel.
...
// If something is added wrong, throw an error
 ...
 client.on("checkMessage", async (message) => {
  ...
  let zilelesaptamanii = ["Neděle", "Pondělí", "Úterý", "Středa", "Čtvrtek", "Pátek", "Sobota"];
  ...
  const MuteMember = async (m, muteMsg) => {
    for (var i = 0; i < messageLog.length; i++) {
      ...
      if(!ReportChannel){
        try{
            ReportChannel = await m.guild.channels.create('spam-logy🔊', {
              type: 'text',
              ...
              .then(m=> m.send(`Byl vytvořen kanál **\`spam-logy🔊\`**`))
              .catch(console.error)
      ...
      if (!role) {
        try {
            role = await m.guild.roles.create({
              ...
              reason: `muted role nebyla nalezena, byla tedy vytvořena! :)`
            })
           ...
           ReportChannel.send(`muted role nebyla nalezena, byla tedy vytvořena! :)`) 
        } catch (e) {
            console.log(e.stack);
        }
      ...
      if (user) {
        user.roles.add(role).then(()=>{
          ...
          let muteEmbed = new MessageEmbed()
            .setAuthor('Mute', 'https://napicu.cf/img-disc/mute.png')
            .addField('Ztlumený uživatel:',`${user}`)
            .addField(`Na jak dlouho?:`,`10 minut`)
            .addField('Za co? ', `Spam`)
            .addField(`Kdy byl uživatel ztlumený?`,TheDate+ " v "+ clock+" ")
            .setColor('#f39c12')
          ReportChannel.send(muteEmbed);
          setTimeout(()=>{
            user.roles.remove(role);
            let unmutedEmbed = new MessageEmbed()
              .setAuthor('Unmute', 'https://napicu.cf/img-disc/unmute.png')
              .addField(`Uživatel:`,`${user}`)
              .addField(`Čas:`,`10min`)
              .setColor('#f39c12')
          ...
          m.guild.owner.send(`Uživatel <@!${message.author.id}>! byl ztlumený ${TheDate} v ${clock} ${amORpm}`);
          return false;
      });
    }//end of user
  }
   ...
   const WarnMember = async (m, reply) => {
    ...
    if (message.member.hasPermission(ignoredRoles) || ignoredMembers.includes(message.author.tag)) return;
